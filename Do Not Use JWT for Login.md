...
```text
Using JWT causes no user track and we have not access to user session controll

If we try to do any things so it make state not stateless and we will add complexty if we try to implement stateless 

If hacker get the token he will have access untiltoken expire by own
```
